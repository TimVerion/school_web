...
public class UpdateAdminServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Admin stu = CommonUtils.toBean(map, Admin.class);//把Map中大部分数据封装到Book对象中
		 ...
		 * 1. 获取真实路径
		 */
		String savepath = this.getServletContext().getRealPath("/images/a_img");
		/*
		 * 2. 创建目标文件
		 */
...
//			 out.close();  
		...
		// 把图片的路径设置给book对象
		stu.setImg("images/a_img/" + filename);
		

		// 调用service完成保存
		Service service = new Service();
		try {
			service.updateAdmin(stu);
	...
	private void error(String msg, HttpServletRequest request, HttpServletResponse response)
		...
		request.getRequestDispatcher("/jsps/admin/admin.jsp").
